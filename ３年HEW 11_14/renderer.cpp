...
#pragma region スワップチェーン（バックバッファ）設定
	// デバイス(DirectXをコントロールする)、スワップチェーン作成
	...
	//よくゲームの設定に置いてあるやつ（FPSゲームとかの）2x MSAA、4x MSAA、8x MSAA
	//swapChainDesc.SampleDesc.Count = 8;(8x MSAA)
...
#pragma endregion
	...
	// デプスステンシルビュー作成 バッファにアクセスするための窓口
	...
	//文字の初期化
	{
		...
		Moji::Instance().InitWithDX11(backBuffer);
		backBuffer->Release();
	}
}
...
void Renderer::Begin() // 描画開始
{
	float clearColor[4] = { 0.9f, 0.8f, 0.9f, 1.0f };
